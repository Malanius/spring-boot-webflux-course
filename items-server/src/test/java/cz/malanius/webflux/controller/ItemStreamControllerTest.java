...
class ItemStreamControllerTest {
    ...
    @Autowired
    ItemStreamControllerTest(ItemReactiveCappedRepository repository, ReactiveMongoOperations mongoOperations, WebTestClient webTestClient) {
    ...
    @Test
    void itemsStream() {
        Flux<ItemCapped> itemFlux = webTestClient.get().uri("/items/stream")
                .exchange()
                .expectStatus().isOk()
